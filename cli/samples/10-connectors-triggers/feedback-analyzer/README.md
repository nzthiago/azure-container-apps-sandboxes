...
[`sandbox-app/server.py`](sandbox-app/server.py) — tweak it for persona,
length, language, or structured output. (The `sandbox-app/` name reflects
that the listener runs **inside the sandbox**, not on your machine.)

## Prerequisites

- The scenario setup applied (`../setup/setup.sh`) — provisions the
