...
| Sandbox group `identity.principalId` is null | `aca sandboxgroup create` does not enable MI. Run `aca sandboxgroup update -g {rg} -n {sg} --identity SystemAssigned` then re-read principalId |
