...
| 11 | [connectors-document-automation](11-connectors-document-automation) | **ACA Sandbox as direct webhook target** for a Connector Namespaces SharePoint trigger. No receiver, no Function host. Inside the sandbox, GitHub Copilot CLI uses the Work IQ SharePoint MCP to fetch the file, `pdftotext`/`tesseract` extract invoice data, and the result is uploaded back to SharePoint via the same MCP. | ✅ ready |
